...
1. 图片按`Section10`，`SectionBig`, `SectionMedium`, `SectionLittle`划分后；
2. 对当前图片P，如果定义为`SectionMedium`和`SectionLittle`，则统计实体截面积 $S<S_T$（现有基牙约$63mm^2$，开放该参数）的实体的数量$N$，如果定义为`Section10`，`SectionBig`,则不处理；
3. 进一步，如果$N>N_T$(可先设定为20，开放该参数) ，则对第2步中实体截面积$S<S_T$的实体识别外接矩形轮廓长$L$和宽$W$；
4. 接着对下一张图片P+1以第2步方式处理，若图片P与图片P+1中已进行外接矩形轮廓识别的对应实体轮廓长$\Delta L$和轮廓宽$\Delta W$在$0.2mm-0.732*L(mm)$  或者 ($0.2mm-0.732*W(mm)$)，此时给图片P的前后$m(5)$张图片打上标签(`SectionJump = True`，默认为`False`)
5. 根据工艺包中的配置参数`SectionJumpVcompensation = xxx`，在打印过程中当前切片层识别到`SectionJump = True`时，对当前工艺包段中的运动速度使用`SectionJumpVcompensation = xxx`降速
