...
2. 对定义为SectionMedium和SectionLittle的截面，统计实体截面积$S<S_T$（现有基牙约$63mm^2$）的截面的数量$N$
3. 如果$N>N_T(20)$ ，则对截面积$S<S_T$的实体识别外接矩形轮廓长$L$和宽$W$；
4. 接着对下一张图片进行上述处理，若该张图片与下一张图片实体轮廓长$\Delta L$和轮廓宽$\Delta W$在$0.2mm-0.732*L(mm)$  或者 ($0.2mm-0.732*W(mm)$)，此时给该张图片的前后$m(5)$张图片打上标签(`SectionJump = True`)
5. 工艺包中配置参数
